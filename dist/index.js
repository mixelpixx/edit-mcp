...
const file_system_manager_1 = require("./file-system/file-system-manager");
const edit_instance_manager_1 = require("./edit-instance/edit-instance-manager");
const operation_router_1 = require("./router/operation-router");
...
if (options.config) {
    try {
        ...
        console.error(chalk_1.default.red(`Error loading configuration file: ${error}`));
        process.exit(1);
    }
}
// Set up logging
const log = {
    ...
    warn: (message) => console.warn(chalk_1.default.yellow(`[WARN] ${message}`)),
...
async function initServer() {
    ...
    const fileSystemManager = new file_system_manager_1.FileSystemManager();
    log.debug('File system manager initialized');
    // Create the Edit instance manager
    const editInstanceManager = new edit_instance_manager_1.EditInstanceManager(config.editExecutable, config.maxEditInstances, config.instanceTimeout);
    log.debug('Edit instance manager initialized');
    // Create the operation router
    const operationRouter = new operation_router_1.OperationRouter(fileSystemManager, editInstanceManager, config.simpleOperationThreshold || 1000, config.complexityFactors);
...
// Register file system tools
function registerFileSystemTools(mcpServer) {
    // Read file tool
...
// Register Edit tools
function registerEditTools(mcpServer) {
    // Format code tool
    mcpServer.registerTool({
        name: 'format_code',
...
// Register hybrid tools
function registerHybridTools(mcpServer, operationRouter) {
    // Smart refactor tool
    mcpServer.registerTool({
        name: 'smart_refactor',
...
function startStdioServer(mcpServer) {
...
async function startHttpServer(mcpServer, port) {
    log.info(`Starting Edit-MCP server with HTTP transport on port ${port}...`);
    const httpServer = new http_server_1.HttpServer(mcpServer, {
        port,
