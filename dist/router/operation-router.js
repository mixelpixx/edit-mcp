...
class OperationRouter {
    constructor(fileSystemManager, editInstanceManager, simpleOperationThreshold = 1000, complexityFactors) {
     ...
     */
    async analyzeFileContext(operation) {
     ...
     */
    analyzePerformanceRequirements(operation) {
     ...
     */
    createBatchStrategy(operation, plan) {
        const batchSize = 50;
        const batches = [];
     ...
     */
    createStreamingStrategy(operation, plan) {
     ...
     */
    async executeWithExecutor(operation, executor) {
        switch (executor) {
            case 'filesystem':
     ...
     */
    async executeWithFileSystem(operation) {
        switch (operation.type) {
            ...
            case 'simple_find_replace':
                return this.fileSystemManager.replaceInFile(operation.params.path, new RegExp(operation.params.pattern, operation.params.flags), operation.params.replacement);
            default:
     ...
     */
    async executeWithEdit(operation) {
     ...
     */
    async executeWithHybrid(operation) {
        switch (operation.type) {
            case 'smart_refactor':
                // Use file system to find occurrences, then Edit for precision
                const searchResults = await Promise.all(operation.affectedFiles.map(file => this.fileSystemManager.findInFile(file, new RegExp(operation.params.oldName, 'g'))));
            ...
            case 'backup_and_edit':
                ...
                const backups = await Promise.all(operation.affectedFiles.map(file => this.fileSystemManager.createBackup(file)));
                try {
                    ...
                    await Promise.all(operation.affectedFiles.map((file, index) => this.fileSystemManager.restoreBackup(backups[index], file)));
                    throw error;
                }
     ...
     */
    async executeParallelBatches(batches) {
        const results = await Promise.all(batches.map(batch => Promise.all(batch.map(operation => this.executeOperation(operation)))));
        return results.flat();
    }
    /**
     * Executes batches of operations sequentially
     */
    async executeSequentialBatches(batches) {
        const results = [];
        for (const batch of batches) {
            const batchResults = await Promise.all(batch.map(operation => this.executeOperation(operation)));
